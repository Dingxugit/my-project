...
import {getDcyhxxBycarnum} from '@/services/swy/dcyh'

const DcyhkpModadl = (props) =>{
  const { modalDckpVisible, onCancel,chejiahao ,riqi } = props;

  const [carInfo,handleInfo] =useState({}) ;
  const [carInfoList,handleInfoList] =useState([]) ;
  useEffect(()=>{
    const lostData =async () =>{
      const response = await getDcyhxxBycarnum({riqi,chepaihaoma: chejiahao});
      if (response.result){
  ...
  >
      <div style={{textAlign: 'center',width: '100%'}}>
        <div style={{fontSize: '25px',fontWeight: 'bolder' ,width: '100%'}}><h1>单车油耗考核卡片</h1></div>
        <div style={{width: '100%'}}>
          <table cellSpacing= '0' border='1px' width="100%">
            <tbody>
              <tr>
                <td width='11.11%'>用油单位</td>
                <td width='33.33%' colSpan={3}>{carInfo==null ? "" :carInfo.yongyoudanwei}</td>
                <td width='11.11%'>车牌号</td>
                <td width='11.11%'>{carInfo ==null? "" :carInfo.chepaihaoma}</td>
