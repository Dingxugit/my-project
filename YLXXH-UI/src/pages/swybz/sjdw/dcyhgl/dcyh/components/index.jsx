...
import {getLastGlsBycarnum, save, update} from "@/services/swy/dcyh";
...
const DcyjglModal = (props) =>{
  ...
  const getLastGls =(e) =>{
    ...
    getLastGlsBycarnum({chepaihaoma:e,year:moment(yearV).format("yyyy")}).then((res) =>{
      if(res.result){
        form.setFieldsValue({
  ...
  const selectDate = (v) =>{
    handleYearV(v);
    if (form.getFieldValue("chepaihaoma") !== undefined && form.getFieldValue("chepaihaoma") !== null){
      getLastGlsBycarnum({chepaihaoma:form.getFieldValue("chepaihaoma"),year:form.getFieldValue("year")}).then((res) =>{
        if(res.result){
          form.setFieldsValue({
            lichengchudu: res.result.lichengweidu === null? 0 : res.result.lichengweidu,
            xinghao:res.result.cpxx
    ...
    >
      <Form
        initialValues={{
          ...
          ,chepaihaoma: values.chepaihaoma
