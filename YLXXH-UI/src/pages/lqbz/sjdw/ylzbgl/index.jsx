...
import {page,getDeptZbz} from "@/services/lqbz/ylzbgl";
...
const handleyear = (v) =>{
  ...
  const columns = [
    ...
    { title: '增拨', dataIndex: 'zengbo' ,hideInSearch : true},
    { title: '转供', dataIndex: 'zhuangong' ,hideInSearch : true},
    { title: '充卡支出', hideInSearch : true,children:[{
  ...
  const handleExport = async () => {
    ...
    const option = {};
    option.fileName = `${dept.name}油料供应指标账`;
    option.datas = [
      {
        sheetData: sheetDataList,
        sheetFilter: [
          'year','numberplate', 'niandufenpei', 'zengbo', 'zhuangong', 'chongkazc', 'unitinformation',
        ],
        sheetHeader,
      },
    ];
  ...
  const handleExportBz = async () =>{
    fetch('/zxHealth/t/lqbz/file/exportYlzb', { // downloadFiles 接口请求地址
      method: 'POST',
      headers: new Headers({
      ...
      response.blob().then(blob => {
        ...
        const filename = `${dept.name}油料供应指标账.xls`;// 设置文件名称
