...
import {Form, Input, InputNumber, message, Modal, Select,Row,Col} from 'antd';
import {getLastGlsBycarnum, save, update} from "@/services/lqbz/dcyhgl";
...
const DcyjglModal = (props) =>{
  ...
  const selectDate = (v) =>{
    ...
    if (form.getFieldValue("numberplate") !== undefined && form.getFieldValue("numberplate") !== null){
      getLastGlsBycarnum({numberplate:form.getFieldValue("numberplate"),year:form.getFieldValue("year")}).then((res) =>{
        if(res.result){
          form.setFieldsValue({
            lichengchudu: res.result.lichengweidu,
            xinghao:res.result.cpxx
          });
        }else{
          form.setFieldsValue({
  ...
  return (
    <Modal
      ...
      destroyOnClose
      title={dcId ? '修改单车油耗考核卡片信息' : '新建单车油耗考核卡片信息'}
    ...
    >
      ...
      >
       ...
       <Col span={12}>
         ...
         >
           {/*<InputNumber style={{width: '80%'}} onChange={handleYear}></InputNumber>*/}
           <DatePicker picker="year" onChange={selectDate} defaultValue={yearV} format="yyyy" style={{width: '80%'}} />
         </FormItem>
        ...
        >
          <InputNumber style={{width: '80%'}} onChange={handleLc} defaultValue={0}></InputNumber>
        </FormItem>
        <FormItem
          {...formItemLayout}
          label="里程末读数"
        ...
        <FormItem
          ...
          name="xingshilicheng"
        >
          <InputNumber style={{width: '80%'}} disabled={true} onChange={handleZxbz}></InputNumber>
        </FormItem>
        <FormItem
          {...formItemLayout}
          label="摩托小时"
        ...
        >
          <InputNumber style={{width: '80%'}} disabled={true}></InputNumber>
        </FormItem>
        <FormItem
          {...formItemLayout}
          label="确认签字"
